...
#### Store Integration Transform URL
FHIR Works on AWS will need to know the URL of the Integration Transform that you would like to send requests to. Please specify the URL by running this command with the correct values filled in for the placeholders.
```
aws ssm put-parameter --region $region --cli-input-json \
'{"Type": "SecureString", "Name": "fhir-service.integration-transform.<AWS-REGION>.<STAGE>.url", "Value": "<IntTranUrl>"}'


Exp

aws ssm put-parameter --region $region --cli-input-json \
'{"Type": "SecureString", "Name": "fhir-service.integration-transform.us-west-2.dev.url", "Value": "http://intTran.com"}'
```
